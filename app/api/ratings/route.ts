...
export async function POST(request: Request) {
  const body = await request.json().catch(() => ({})) as { runId?: unknown; listenerId?: unknown; naturalness?: unknown; speakerJudgment?: unknown };
  ...
  if (speakerJudgment !== "same" && speakerJudgment !== "unsure" && speakerJudgment !== "different") return NextResponse.json({ error: "Choose a speaker comparison." }, { status: 400 });
  appendRating({ runId, createdAt: new Date().toISOString(), listenerId, naturalness, speakerJudgment });
