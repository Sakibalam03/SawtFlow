...
type Run = { runId: string; createdAt: string; category: string; language: Language; text: string; status: "ok" | "error"; audioFile?: string; audioSeconds?: number; generationSeconds?: number; fullClipLatencySeconds?: number; rtf?: number; peakVramMb?: number; error?: string; evaluation?: Evaluation | null; ratings?: RatingSummary };
type Metrics = { latest: (Run & { ratings: RatingSummary; evaluation: Evaluation | null }) | null; summary: { attemptedRuns: number; successfulRuns: number; failureRate: number | null; medianFullClipSeconds: number | null; p95FullClipSeconds: number | null; medianRtf: number | null; p95Rtf: number | null; maxPeakVramMb: number | null; meanWer: number | null; meanSpeakerCosine: number | null }; history: Run[] };
...
const profiles = {
  ...
  names_numbers: { label: "Names and numbers", text: "Dr. Maya Chen will meet Omar in room 407 at 4:15 PM." },
  prosody: { label: "Prosody - question", text: "Are you certain? I said Tuesday, not Thursday - please check again." },
} as const;
const languageDefaults: Record<Language, string> = { en: profiles.free_text.text, hi: "नमस्ते, यह आपकी आवाज़ में हिंदी का एक नमूना है।", ar: "مرحباً، هذا نموذج صوتي باللغة العربية بصوتك." };
const languageNames: Record<Language, string> = { en: "English", hi: "Hindi", ar: "Arabic" };
...
export default function Home() {
  ...
  const [evaluatorStatus, setEvaluatorStatus] = useState<"warming" | "ready" | "error">("warming");
  ...
  const [rating, setRating] = useState({ listenerId: "", naturalness: "", speakerJudgment: "unsure" });
  ...
  useEffect(() => {
    void refreshMetrics();
    void fetch("/api/generate", { cache: "no-store" }).then(response => { if (!response.ok) throw new Error(); setWorkerStatus("ready"); }).catch(() => setWorkerStatus("error"));
    void fetch("/api/evaluate", { cache: "no-store" }).then(response => { if (!response.ok) throw new Error(); setEvaluatorStatus("ready"); }).catch(() => setEvaluatorStatus("error"));
  }, [refreshMetrics]);
  ...
  const generate = async () => {
    ...
    try {
      const response = await fetch("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, category: profile, language }) });
  ...
  const evaluate = async () => {
    ...
    try {
      ...
      setEvaluatorStatus("ready"); await refreshMetrics(currentRunId);
  ...
  const submitRating = async () => {
    ...
    try {
      ...
      const result = await response.json() as { error?: string };
      if (!response.ok) throw new Error(result.error || "Could not save listener evidence.");
      setRating(current => ({ ...current, naturalness: "" })); await refreshMetrics(currentRunId);
    } catch (ratingError) { setError(ratingError instanceof Error ? ratingError.message : "Could not save listener evidence."); }
    finally { setSavingRating(false); }
  };

  const active = metrics?.latest?.runId === currentRunId ? metrics.latest : null;
  ...
  return <main className="page evaluation-page">
    ...
    <div className="workspace-grid">
      <div className="generation-column">
        <section className="studio"><div className="studio-header"><div><h2>Generation input</h2><p>Choose a profile for comparable evidence, or write your own text.</p></div><span>{text.trim().length} characters</span></div><div className="language-switcher"><span>Voice language</span><div>{(Object.keys(languageNames) as Language[]).map(code => <button key={code} className={language === code ? "active" : ""} onClick={() => { setLanguage(code); setProfile("free_text"); setText(languageDefaults[code]); setAudioUrl(null); if (status === "ready") setStatus("idle"); }}>{languageNames[code]}</button>)}</div></div><div className="profile-row"><label>Prompt profile<select value={profile} onChange={event => { const next = event.target.value as keyof typeof profiles; setProfile(next); setText(language === "en" ? profiles[next].text : languageDefaults[language]); }}><option value="free_text">Free text</option><option value="latency">Latency - short reply</option><option value="names_numbers">Names and numbers</option><option value="prosody">Prosody - question</option></select></label><span>{languageNames[language]} - {language === "en" ? "Turbo" : "Multilingual"} pipeline</span></div><textarea value={text} onChange={event => { setText(event.target.value); setProfile("free_text"); setAudioUrl(null); if (status === "ready") setStatus("idle"); }} maxLength={1000} aria-label="Text to convert to audio" /><div className="studio-actions"><button className="generate" onClick={generate} disabled={!text.trim() || status === "processing" || (language === "en" && workerStatus === "warming")}>{status === "processing" ? <><span className="spinner" /> Generating {languageNames[language]} audio...</> : language === "en" && workerStatus === "warming" ? <><span className="spinner" /> Warming voice model...</> : <><Icon name="spark" size={16} /> Generate audio</>}</button><span>Up to 1,000 characters</span></div>{error && <p className="error" role="alert">{error}</p>}</section>
        <section className={`audio-card ${status === "processing" ? "is-processing" : ""} ${isPlaying ? "is-playing" : ""}`} aria-live="polite"><div className="audio-visual"><span className="halo halo-one" /><span className="halo halo-two" /><div className="mic"><Icon name="mic" size={27} /></div></div><div className="audio-copy">{status === "processing" ? <><p className="label">{language === "en" ? "CHATTERBOX TURBO" : "CHATTERBOX MULTILINGUAL"}</p><h2>Creating your {languageNames[language]} voice sample</h2><span>Your voice model is generating audio...</span><div className="loading-wave">{Array.from({ length: 18 }).map((_, index) => <i key={index} />)}</div></> : active ? <><p className="label ready-label"><Icon name="check" size={13} /> AUDIO READY</p><h2>{isPlaying ? "Playing your voice sample" : "Your voice sample is ready"}</h2><span><b>Audio length:</b> {fmt(active.audioSeconds)} s <em>-</em> <b>Generated in:</b> {fmt(active.generationSeconds)} s</span></> : <><p className="label">AUDIO PREVIEW</p><h2>{language === "en" && workerStatus === "warming" ? "Preparing your voice model" : "Ready when you are"}</h2><span>{language === "en" && workerStatus === "warming" ? "Turbo is loading once so future requests are faster." : `Generate ${languageNames[language]} audio to create a voice-cloned WAV.`}</span></>}</div>{audioUrl && <><audio ref={audio} src={audioUrl} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onEnded={() => setIsPlaying(false)} /><button className="play" onClick={() => void playAudio()} aria-label={isPlaying ? "Stop audio" : "Play audio"}>{isPlaying ? <Icon name="stop" size={17} /> : <Icon name="play" size={17} />}</button></>}</section>
      </div>

      <aside className="evidence-column">
        <section className="metrics-panel"><div className="panel-heading"><div><p className="label"><Icon name="chart" size={14} /> EVALUATION</p><h2>Evidence for this clip</h2></div>{currentRunId && <button className="secondary" onClick={evaluate} disabled={evaluating}>{evaluating ? "Evaluating..." : evaluatorStatus === "warming" ? "Evaluator warming..." : "Run objective evaluation"}</button>}</div><div className="metric-grid objective-metrics"><Metric title="Naturalness MOS" value={active?.ratings.naturalnessMos ? `${fmt(active.ratings.naturalnessMos)}/5` : "Pending"} note={`${active?.ratings.listenerCount ?? 0} listeners - target >= 4.0`} state={pass(active?.ratings.naturalnessMos, value => value >= 4)} /><Metric title="Speaker cosine" value={active?.evaluation?.speakerCosine ? fmt(active.evaluation.speakerCosine, 3) : "Pending"} note="ECAPA reference similarity - target >= 0.75" state={pass(active?.evaluation?.speakerCosine, value => value >= .75)} /><Metric title="Round-trip WER" value={typeof active?.evaluation?.wer === "number" ? `${(active.evaluation.wer * 100).toFixed(1)}%` : "Pending"} note="ASR vs input text - target <= 10%" state={pass(active?.evaluation?.wer, value => value <= .10)} /><Metric title="Full-clip latency" value={active ? `${fmt(active.fullClipLatencySeconds)} s` : "Pending"} note="Batch API - target < 2.0 s" state={pass(active?.fullClipLatencySeconds, value => value < 2)} /><Metric title="Real-time factor" value={active ? fmt(active.rtf) : "Pending"} note="Generation / audio length - target <= 0.5" state={pass(active?.rtf, value => value <= .5)} /><Metric title="Listener similarity" value={active?.ratings.sameSpeakerRate !== null && active?.ratings.sameSpeakerRate !== undefined ? `${(active.ratings.sameSpeakerRate * 100).toFixed(0)}% same` : "Pending"} note={`${active?.ratings.ratingsCount ?? 0} listener comparisons`} state={active?.ratings.sameSpeakerRate ? "pass" : "pending"} /></div></section>
        {currentRunId && <section className="listener-panel compact-listener"><div className="panel-heading"><div><p className="label"><Icon name="listen" size={14} /> LISTENER EVIDENCE</p><h2>Quick reference check</h2></div><audio controls src="/api/reference" /></div><div className="compact-rating"><input value={rating.listenerId} onChange={event => setRating(current => ({ ...current, listenerId: event.target.value }))} placeholder="Listener ID" /><select value={rating.naturalness} onChange={event => setRating(current => ({ ...current, naturalness: event.target.value }))}><option value="">Naturalness 1-5</option><option value="1">1 - very artificial</option><option value="2">2 - mostly artificial</option><option value="3">3 - acceptable</option><option value="4">4 - natural</option><option value="5">5 - very natural</option></select><select value={rating.speakerJudgment} onChange={event => setRating(current => ({ ...current, speakerJudgment: event.target.value }))}><option value="same">Same speaker</option><option value="unsure">Unsure</option><option value="different">Different speaker</option></select><button className="secondary" onClick={submitRating} disabled={savingRating || !rating.listenerId || !rating.naturalness}>{savingRating ? "Saving..." : "Save"}</button></div></section>}
        <section className="history-panel"><div className="panel-heading"><div><p className="label"><Icon name="chart" size={14} /> RUN HISTORY</p><h2>Current runtime summary</h2></div><span>{metrics?.summary.successfulRuns ?? 0} successful / {metrics?.summary.attemptedRuns ?? 0} attempts</span></div><div className="summary-grid"><Summary label="Median / p95 latency" value={`${fmt(metrics?.summary.medianFullClipSeconds)} / ${fmt(metrics?.summary.p95FullClipSeconds)} s`} /><Summary label="Median / p95 RTF" value={`${fmt(metrics?.summary.medianRtf)} / ${fmt(metrics?.summary.p95Rtf)}`} /><Summary label="Mean WER" value={typeof metrics?.summary.meanWer === "number" ? `${(metrics.summary.meanWer * 100).toFixed(1)}%` : "Pending"} /><Summary label="Mean speaker cosine" value={fmt(metrics?.summary.meanSpeakerCosine, 3)} /><Summary label="Peak GPU memory" value={metrics?.summary.maxPeakVramMb ? `${fmt(metrics.summary.maxPeakVramMb, 0)} MB` : "Pending"} /><Summary label="Failure rate" value={typeof metrics?.summary.failureRate === "number" ? `${(metrics.summary.failureRate * 100).toFixed(0)}%` : "Pending"} /></div><div className="history-list">{metrics?.history.length ? metrics.history.map(run => <div key={run.runId} className="history-row"><span className={run.status === "ok" ? "dot success" : "dot failure"} /><div><strong>{languageNames[run.language as Language] ?? run.language} - {profiles[run.category as keyof typeof profiles]?.label ?? run.category}</strong><small>{new Date(run.createdAt).toLocaleString()} - {run.status === "ok" ? `${fmt(run.fullClipLatencySeconds)} s - RTF ${fmt(run.rtf)}` : run.error}</small></div><span>{run.evaluation?.status === "ok" ? `WER ${(run.evaluation.wer! * 100).toFixed(1)}%` : "Not evaluated"}</span></div>) : <p className="empty-history">Generate a clip to begin collecting evidence.</p>}</div></section>
      </aside>
    </div>
    <p className="footnote"><Icon name="mic" size={14} /> Runtime telemetry and objective results are stored locally under <code>outputs/ui</code>.</p>
  </main>;
}
