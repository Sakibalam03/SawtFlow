...
const session = globalThis as typeof globalThis & {
  infiniaUiEvidenceSession?: boolean;
};
...
export function ensureUiEvidenceSession() {
  ...
  for (const file of [runsPath, evaluationsPath, ratingsPath])
...
export type UiRun = {
  runId: string;
  createdAt: string;
  model: "chatterbox-turbo" | "chatterbox-multilingual" | "indicf5";
  language: "en" | "hi" | "ar";
  category: string;
  text: string;
  audioFile?: string;
...
export type UiRating = {
  runId: string;
  createdAt: string;
  listenerId: string;
  naturalness: number;
  speakerJudgment: "same" | "unsure" | "different";
  comment?: string;
};
...
const values = (items: Array<number | null | undefined>) =>
  items.filter(
    (item): item is number => typeof item === "number" && Number.isFinite(item),
  );
const mean = (items: number[]) =>
  items.length
    ? round(items.reduce((sum, item) => sum + item, 0) / items.length)
    : null;
...
export function readRuns() {
  return rows<UiRun>(runsPath);
}
export function appendRun(run: UiRun) {
  append(runsPath, run);
}
export function readEvaluations() {
  return rows<UiEvaluation>(evaluationsPath);
}
export function appendEvaluation(evaluation: UiEvaluation) {
  append(evaluationsPath, evaluation);
}
export function readRatings() {
  return rows<UiRating>(ratingsPath);
}
export function appendRating(rating: UiRating) {
  append(ratingsPath, rating);
}
...
export function metricsFor(runId?: string) {
  ...
  for (const evaluation of evaluations)
    latestEvaluation.set(evaluation.runId, evaluation);
  const relevantRuns = runId ? runs.filter((run) => run.runId === runId) : runs;
  const success = relevantRuns.filter((run) => run.status === "ok");
  const selected = runId
    ? runs.findLast((run) => run.runId === runId)
    : runs.at(-1);
  const selectedEvaluation = selected
    ? latestEvaluation.get(selected.runId)
    : undefined;
  const selectedRatings = selected
    ? ratings.filter((rating) => rating.runId === selected.runId)
    : [];
  const same = selectedRatings.filter(
    (rating) => rating.speakerJudgment === "same",
  ).length;
  ...
  const objective = [...latestEvaluation.values()].filter(
    (item) => item.status === "ok" && (!runId || relevantIds.has(item.runId)),
  );
  ...
  return {
    ...
    latest: selected
      ? {
          ...
          ratings: {
            naturalnessMos: mean(
              selectedRatings.map((rating) => rating.naturalness),
            ),
            ratingsCount: selectedRatings.length,
            listenerCount: listeners.size,
            sameSpeakerVotes: same,
            sameSpeakerRate: selectedRatings.length
              ? round(same / selectedRatings.length)
              : null,
          },
        }
      : null,
    summary: {
      attemptedRuns: relevantRuns.length,
      successfulRuns: success.length,
      failureRate: relevantRuns.length
        ? round((relevantRuns.length - success.length) / relevantRuns.length)
        : null,
      medianFullClipSeconds: median(timing),
      p95FullClipSeconds: p95(timing),
      medianRtf: median(rtf),
      p95Rtf: p95(rtf),
      maxPeakVramMb: vram.length ? Math.max(...vram) : null,
      meanWer: mean(values(objective.map((item) => item.wer))),
      meanSpeakerCosine: mean(
        values(objective.map((item) => item.speakerCosine)),
      ),
      evaluatedRuns: objective.length,
    },
    history: runs
      .slice(-12)
      .reverse()
      .map((run) => ({
        ...run,
        evaluation: latestEvaluation.get(run.runId) ?? null,
      })),
  };
}
