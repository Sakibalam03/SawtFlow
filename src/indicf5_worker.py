...
def main() -> None:
    ...
    for line in sys.stdin:
        try:
            ...
            reference_transcript = str(request.get("referenceTranscript", config["reference_transcript"])).strip()
            if not text or len(text) > 1000: raise ValueError("Text must contain 1 to 1,000 characters.")
            if not requested_reference.is_file(): raise FileNotFoundError("Selected voice reference WAV is missing.")
            if not reference_transcript: raise ValueError("Selected voice reference transcript is required.")
            reset_peak_memory(device); cuda_synchronize(device); started = time.perf_counter()
            waveform = model(text, ref_audio_path=str(requested_reference), ref_text=reference_transcript)
